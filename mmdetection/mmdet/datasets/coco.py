        ...
        for metric in metrics:
            ...
            if metric == 'proposal':
                ...
                for i, item in enumerate(metric_items):
                    val = float('{:.6f}'.format(cocoEval.stats[i + 6]))
                ...
                for i in range(len(metric_items)):
                    ...
                    val = float('{:.6f}'.format(cocoEval.stats[i]))
                    eval_results[key] = val
                eval_results['{}_mAP_copypaste'.format(metric)] = (
                    '{ap[0]:.6f} {ap[1]:.6f} {ap[2]:.6f} {ap[3]:.6f} '
                    '{ap[4]:.6f} {ap[5]:.6f}').format(ap=cocoEval.stats[:6])
        if tmp_dir is not None:
            tmp_dir.cleanup()
        return eval_results, cocoEval.stats, cocoEval.aps
